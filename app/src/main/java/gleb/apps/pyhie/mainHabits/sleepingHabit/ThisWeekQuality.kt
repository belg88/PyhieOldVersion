...
class ThisWeekQuality : Fragment() {
    private val TAG = "TAG"
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_seven_day_efficiency, container, false)
        ...
        lifecycleScope.launch {
            ...
            when (habitData) {
                Keys.SLEEP_DATA -> {
                    val sleepingList = FirebaseService.getSleepingDataThisWeek(email!!)
                ...
                Keys.EATING_DATA -> {
                    val eatingList = FirebaseService.getEatingDataThisWeek(email!!)
                    if (eatingList != null) {
                ...
                Keys.CLEANING_DATA -> {
                    val cleaningList = FirebaseService.getCleaningDataThisWeek(email!!)
    ...
    private fun setUpProgressView(list: List<Any>, averageQuality: Double) {
        quality.text = "${averageQuality.toInt()}%"
